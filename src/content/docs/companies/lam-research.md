---
title: "Lam Research"
slug: "companies/lam-research"
description: "Lam Research 在这里代表半导体晶圆制造设备，核心变量是刻蚀、沉积、先进制程、存储周期和晶圆厂资本开支。它是 AI 价值链里“算力需求如何变成设备订单”的一层。"
---

> **争议与共识**  
> Lam Research 在这里代表半导体晶圆制造设备，核心变量是刻蚀、沉积、先进制程、存储周期和晶圆厂资本开支。它是 AI 价值链里“算力需求如何变成设备订单”的一层。

## 简介

Lam Research 在这里代表半导体晶圆制造设备，核心变量是刻蚀、沉积、先进制程、存储周期和晶圆厂资本开支。它是 AI 价值链里“算力需求如何变成设备订单”的一层。

## 公司点评

### [菲利普·拉丰](/investor-masters/investors/philippe-laffont/) / [Coatue Management](/investor-masters/institutions/coatue-management/)

Coatue 在 `2026Q1` 持有 `LRCX` 约 `10.04m` 股、`$2.15bn`，公开 13F 权重 `7.4%`，位列第三大仓。股数较 `2025Q4` 的约 `9.81m` 小幅增加，说明它不是单纯价格推高后的前排仓位。

在 [13F趋势-Coatue](/investor-masters/institutions/13f-trends-coatue/) 里，`LRCX` 要和 `TSM / AMAT / AVGO / ASML` 一起读：Coatue 把 AI 的投资表达继续拆到先进制造和设备资本开支。

## 相关页面

- [菲利普·拉丰](/investor-masters/investors/philippe-laffont/)
- [Coatue Management](/investor-masters/institutions/coatue-management/)
- [13F趋势-Coatue](/investor-masters/institutions/13f-trends-coatue/)
- [台积电](/investor-masters/companies/tsmc/)
- [ASML](/investor-masters/companies/asml/)
- [Applied Materials](/investor-masters/companies/applied-materials/)
- [Broadcom](/investor-masters/companies/broadcom/)
