---
title: "Applied Materials"
slug: "companies/applied-materials"
description: "Applied Materials 在这批资料里是半导体制造设备平台的代表，覆盖沉积、刻蚀、检测和先进封装等晶圆厂关键环节。它对应的是 AI 算力需求背后的制造资本开支。"
---

> **争议与共识**  
> Applied Materials 在这批资料里是半导体制造设备平台的代表，覆盖沉积、刻蚀、检测和先进封装等晶圆厂关键环节。它对应的是 AI 算力需求背后的制造资本开支。

## 简介

Applied Materials 在这批资料里是半导体制造设备平台的代表，覆盖沉积、刻蚀、检测和先进封装等晶圆厂关键环节。它对应的是 AI 算力需求背后的制造资本开支。

## 公司点评

### [菲利普·拉丰](/investor-masters/investors/philippe-laffont/) / [Coatue Management](/investor-masters/institutions/coatue-management/)

Coatue 在 `2026Q1` 持有 `AMAT` 约 `5.25m` 股、`$1.79bn`，公开 13F 权重 `6.2%`，进入前十。但股数低于 `2025Q4` 的约 `5.98m`，所以这更像组合权重和前十重排后的核心留存，不应直接写成主动加仓。

和 `TSM / LRCX / AVGO / ASML` 放在一起看，[13F趋势-Coatue](/investor-masters/institutions/13f-trends-coatue/) 展示的是一条先进制造和半导体设备链，而不是单点押注。

## 相关页面

- [菲利普·拉丰](/investor-masters/investors/philippe-laffont/)
- [Coatue Management](/investor-masters/institutions/coatue-management/)
- [13F趋势-Coatue](/investor-masters/institutions/13f-trends-coatue/)
- [台积电](/investor-masters/companies/tsmc/)
- [ASML](/investor-masters/companies/asml/)
- [Lam Research](/investor-masters/companies/lam-research/)
- [Broadcom](/investor-masters/companies/broadcom/)
