---
title: "H&H International Investment"
slug: "institutions/h-h-international-investment"
description: "H&H International Investment 的机构级哲学、文化与传承。"
---
## 机构简介
...
- [段永平](/investor-masters/investors/duan-yongping/)：H&H 在当前资料中的核心人物，也是这套方法语言的主要来源。

## 标志性公开材料

- [段永平：抄捷径是阴沟里翻船最快的方法](/investor-masters/sources/source-131/)：理解 `本分 / 平常心 / Stop Doing List` 的核心材料。
- [“打孔机”只打了不到10个孔！段永平与方三文最新对话，详解“投资中如何真正算看懂”……](/investor-masters/sources/source-92/)：理解 `看懂生意 / 打孔机 / 极少出手` 的核心材料。
...
- 持仓总市值从 `$20.00bn` 降至 `$19.10bn`，减少 `$903.4m`（`-4.5%`）。粗拆：主动变化约 `-$1.88bn`，价格变化约 `+$0.98bn`。
- 大幅减持 NVDA `-54.6%`、GOOG `-46.9%`、UNH `-33.6%`、MSFT `-25.8%`，同时小幅减持 AAPL `-6.4%`；清仓 TSM 与 CRWV。
- 逆势加仓 PDD `+26.7%`；BRK.B `+0.9%`，TSLA、OXY基本不动，另有小额新建 BABA。
...
- `AAPL`：`41.05%`，约 `$7.841bn`，股数 `27,098,707`，较上季 `-6.4%`。
- `BRK.B`：`24.18%`，约 `$4.618bn`，股数 `9,229,696`，较上季 `+0.9%`。
- `PDD`：`9.99%`，约 `$1.909bn`，股数 `25,022,094`，较上季 `+26.7%`。
- `TSLA`：`7.44%`，约 `$1.422bn`，股数 `3,380,400`，较上季 `-0.8%`。
- `NVDA`：`6.58%`，约 `$1.257bn`，股数 `6,280,675`，较上季 `-54.6%`。
- `GOOG`：`3.64%`，约 `$695.6m`，股数 `1,968,600`，较上季 `-46.9%`。
- `OXY`：`2.61%`，约 `$498.4m`，股数 `10,261,500`，较上季不变。
- `MSFT`：`1.47%`，约 `$281.3m`，股数 `754,100`，较上季 `-25.8%`。
- `CRDO`：`1.00%`，约 `$190.7m`，股数 `701,200`，较上季 `-6.7%`。
- `UNH`：`0.87%`，约 `$166.1m`，股数 `399,600`，较上季 `-33.6%`。

#### Q1 2026

- `AAPL` Apple Inc（消费电子与服务平台，iPhone 是核心入口，服务业务贡献高毛利和生态粘性）：`13F 占比 36.7%`，市值约 `$7.35bn`，股数/份额 `28,945,607`。
- `BRK.B` Berkshire Hathaway Inc Del（伯克希尔哈撒韦，保险 float、全资经营业务和公开股票组合构成的复合资本配置平台）：`13F 占比 21.9%`，市值约 `$4.38bn`，股数/份额 `9,147,796`。
- `NVDA` Nvidia Corporation（AI 加速计算平台，GPU、网络和 CUDA 生态共同支撑数据中心算力需求）：`13F 占比 12.1%`，市值约 `$2.41bn`，股数/份额 `13,843,775`。
- `PDD` Pdd Holdings Inc（拼多多和 Temu 背后的电商平台，核心变量是低价供给、流量效率和跨境扩张）：`13F 占比 10.1%`，市值约 `$2.02bn`，股数/份额 `19,748,294`。
- `TSLA` Tesla Inc（特斯拉，电动车、储能和自动驾驶平台公司，核心变量是制造效率、FSD 与 Robotaxi 期权）：`13F 占比 6.3%`，市值约 `$1.27bn`，股数/份额 `3,408,900`。
- `GOOG` Alphabet Inc（Alphabet 的 C 类股，核心资产是 Google 搜索、YouTube、广告网络、Android 和 Google Cloud）：`13F 占比 5.3%`，市值约 `$1.06bn`，股数/份额 `3,706,000`。
- `OXY` Occidental Pete Corp（西方石油，上游油气生产商，现金流高度受油价、资本开支和资产负债表约束影响）：`13F 占比 3.3%`，市值约 `$0.67bn`，股数/份额 `10,261,500`。
- `MSFT` Microsoft Corp（微软，企业软件、Azure 云、Office、Windows 和 AI 基础设施共同构成的生产力平台）：`13F 占比 1.9%`，市值约 `$0.38bn`，股数/份额 `1,016,000`。
- `UNH` Unitedhealth Group Inc（联合健康，医保和医疗服务平台公司，核心看医疗成本率、政策风险和 Optum 服务能力）：`13F 占比 0.8%`，市值约 `$0.16bn`，股数/份额 `601,400`。
- `DIS` Disney Walt Co（迪士尼，内容 IP、主题乐园、体育媒体和流媒体组成的娱乐资产组合）：`13F 占比 0.7%`，市值约 `$0.15bn`，股数/份额 `1,511,800`。
