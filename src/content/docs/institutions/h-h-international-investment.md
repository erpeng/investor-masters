...
- [段永平：抄捷径是阴沟里翻船最快的方法](/investor-masters/sources/source-125/)：理解 `本分 / 平常心 / Stop Doing List` 的核心材料。
- [“打孔机”只打了不到10个孔！段永平与方三文最新对话，详解“投资中如何真正算看懂”……](/investor-masters/sources/source-87/)：理解 `看懂生意 / 打孔机 / 极少出手` 的核心材料。
...
- 持仓总市值变化：从 `$17.49bn` 到 `$20.00bn`，上升 `$2.52bn`（+14.4%）。粗拆：主动仓位变化 `$3.67bn`，价格变化 `-$1.16bn`；主要由主动仓位变化推升。
- 新进前 10：TSLA, UNH
- 退出前 10：BABA, TSM
- 权重上升：NVDA（+4.4 pct）, PDD（+2.6 pct）, GOOG（+2.0 pct）, BRK.B（+1.3 pct）
- 权重下降：AAPL（-13.6 pct）, TSM（-1.9 pct）, MSFT（-0.5 pct）
